...
import { resolvePostCategories, normalizeTagToSlug, getTagLabel, getLocalizedCategoryLabel } from '@/features/content/lib/post-categories'
...
function ListLayoutWithTagsInner({
  ...
  const currentFilterKey = `${locale}-${sortOrder}-${currentTagSlug || ''}`
    ...
    <section className="mx-auto max-w-5xl px-4 pt-4 pb-12 sm:pt-6 sm:pb-16 sm:px-6 lg:px-8">
      <div className="space-y-6 sm:space-y-8">
          <PageHeader
            ...
            meta={
              !!currentTagSlug && (
                <span className="text-neutral-6">{dictionary.archive.postCount.replace('{count}', String(currentTagCount))}</span>
              )
            }
            action={
        ...
        <div className="space-y-6">
          ...
          >
            {blogListActive ? (
              ...
              <Link
                href="/blog"
                className="hover:bg-neutral-2 border border-border/50 inline-flex shrink-0 rounded-full px-3 py-1 text-label-12 font-normal text-neutral-7 transition-colors hover:text-neutral-10"
              >
                {dictionary.common.allPosts}
              </Link>
            )}
