  ...
  return (
    <div className="mt-6 flex justify-center sm:mt-10">
      <nav
        ...
        className="border-border/30 bg-background/60 relative flex items-center justify-center gap-1 rounded-full border p-1.5 shadow-sm backdrop-blur-xl sm:gap-2 sm:p-2"
      >
        {prevPage ? (
          <Link
            href={getPageHref(basePath, currentPage - 1, queryString)}
            rel="prev"
            className="text-muted-foreground/40 hover:bg-primary/8 hover:text-primary flex h-9 w-9 items-center justify-center rounded-full transition-all hover:scale-110 sm:h-10 sm:w-10"
            aria-label={dictionary.common.previous}
          >
            <ChevronLeft size={18} strokeWidth={2.5} />
          </Link>
        ) : (
          <span className="text-muted-foreground/10 flex h-9 w-9 items-center justify-center rounded-full sm:h-10 sm:w-10">
            <ChevronLeft size={18} strokeWidth={2.5} />
          </span>
        )}
          ...
          {items.map((item, index) => {
            if (typeof item !== 'number') {
              return (
                <span
                  key={`${item}-${index}`}
                  className="text-muted-foreground/20 w-4 text-center text-[10px] font-black"
                >
                  ...
                </span>
              )
