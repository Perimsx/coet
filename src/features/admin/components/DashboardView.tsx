...
export function DashboardView() {
  ...
  const load = () => {
    ...
    cmsApi
      ...
      .then(setSummary)
      .catch(() =>
        setError('无法加载仪表盘数据，请确认后端 CMS API 服务已启动。')
      )
  }
  ...
  const quickActions = [
    {
      label: '撰写新文章',
      icon: PenTool,
      href: '/admin/content/posts?create=true',
      color: 'text-primary',
    },
    {
      label: '评论审核',
      icon: MessageSquare,
      href: '/admin/engagement/comments',
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {cards.map((card) => (
          <Card
            key={card.label}
            className="border border-zinc-200/80 dark:border-zinc-800/80 bg-white dark:bg-zinc-900 rounded-lg overflow-hidden hover:border-zinc-300 dark:hover:border-zinc-700 transition-all"
          >
            <CardBody className="p-3 flex items-center justify-between">
              ...
              <div className="p-2 rounded-md bg-zinc-100 dark:bg-zinc-800/80 shrink-0">
                {card.icon}
              </div>
            </CardBody>
          </Card>
          ...
          {quickActions.map((action) => {
            ...
            return (
              <Button
                ...
                className="w-full justify-between p-3 text-left group"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        <Card className="lg:col-span-7 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-lg">
          ...
          <CardBody className="p-3 text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed space-y-2">
            <p>
              本系统采用前后台深度集成的 MDX 与 JSON
              存储机制。文章、分类、标签与全站设置统一沉淀为可版本的 Git
              离线文件。
            </p>
            <p className="text-zinc-500">
              评论与管理员审计日志持久化于轻量级本地 SQLite
              中，兼顾了无缝的离线编辑体验与强大的动态交互能力。
            </p>
          </CardBody>
        </Card>

        <Card className="lg:col-span-5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-lg">
          <CardHeader className="font-bold text-sm border-b border-zinc-100 dark:border-zinc-800/60 pb-2.5">
            标准发布流程
          </CardHeader>
          <CardBody className="p-3 flex flex-col gap-2 text-xs text-zinc-600 dark:text-zinc-400">
            <div className="flex items-start gap-2">
              <span className="grid w-4 h-4 place-items-center rounded-full bg-primary/10 text-primary font-mono text-[10px] font-bold shrink-0 mt-0.5">
                1
              </span>
