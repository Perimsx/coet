...
function getStatusView(status: SuggestionStatus) {
  ...
  if (status === "replied") {
    return {
      label: "已回复",
      className: "rounded-full border-none bg-sky-500/15 text-sky-700 dark:text-sky-300",
    }
  }
  ...
  if (status === "in_progress") {
    return {
      label: "跟进中",
      className: "rounded-full border-none bg-violet-500/15 text-violet-700 dark:text-violet-300",
    }
  }
...
export default function SuggestionsClient({
  ...
  const handleStatusChange = (item: Suggestion, status: SuggestionStatus) => {
    startTransition(async () => {
      ...
      setData((current) =>
        current.map((entry) => (entry.id === item.id ? result.item || entry : entry))
      )
  ...
  return (
    <div className="space-y-5">
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <AdminStatCard title="建议总数" value={stats.total} hint="全部访客反馈工单" icon={MessageSquare} />
        <AdminStatCard title="待处理" value={stats.pending} hint="需要尽快分派或回复" icon={Search} />
        <AdminStatCard title="跟进中" value={stats.inProgress} hint="正在处理的问题与需求" icon={RefreshCw} />
        <AdminStatCard title="已解决" value={stats.resolved} hint="处理完成并可归档" icon={Send} />
      </section>
                    ...
                    <AdminPanelBody className="space-y-5 p-5">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="text-xs text-muted-foreground">
                              最近更新 {new Date(item.updatedAt || item.createdAt).toLocaleString("zh-CN")}
                            </div>
                          </div>
                        </div>
                      ...
                      {item.adminReply ? (
                        <div className="rounded-[24px] border border-sky-500/20 bg-sky-500/5 p-4">
                          <div className="mb-2 text-sm font-medium text-foreground">最近回复</div>
      ...
      >
        <DialogContent className="rounded-[28px] border-border/70 p-0 sm:max-w-3xl">
          <DialogHeader className="border-b border-border/60 px-6 py-5 text-left">
            ...
            <DialogDescription>模板、回复内容和结果状态都在这里一次完成。</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 px-6 py-5">
            ...
            {templates.length ? (
              <div className="space-y-2">
                <div className="text-sm font-medium text-foreground">快捷模板</div>
            ...
            <div className="grid gap-4 md:grid-cols-[1fr_180px]">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">回复后状态</label>
                <Select
                  value={replyStatus}
                  onValueChange={(value) => setReplyStatus(value as SuggestionStatus)}
                >
      ...
      <Dialog open={templateDialogOpen} onOpenChange={setTemplateDialogOpen}>
        <DialogContent className="rounded-[28px] border-border/70 p-0 sm:max-w-3xl">
          <DialogHeader className="border-b border-border/60 px-6 py-5 text-left">
            ...
            <DialogDescription>把高频话术收成模板，回复时可以一键插入。</DialogDescription>
          </DialogHeader>
          <div className="grid gap-5 px-6 py-5 lg:grid-cols-[280px_1fr]">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Button type="button" className="rounded-xl" disabled={pending} onClick={handleSaveTemplate}>
                  保存模板
                </Button>
                {templateDraft.id ? (
                  <Button
                    ...
                    onClick={() =>
                      startTransition(async () => {
                        const result = await deleteSuggestionTemplateAction(templateDraft.id)
                        if (!result.ok) {
                          toast.error(result.error)
                          return
                        }
                        ...
                        setTemplates((current) =>
                          current.filter((item) => item.id !== templateDraft.id)
                        )
                        setTemplateDraft({ id: 0, title: "", content: "" })
                        toast.success("模板已删除")
                      })
